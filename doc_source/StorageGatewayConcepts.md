...
To use the file gateway, you start by downloading a VM image for the file storage gateway\. You then activate the file gateway from the AWS Management Console or through the Storage Gateway API\. File gateway is also available as an Amazon EC2 images\.

After the file gateway is activated, you create and configure your file share and associate that share with your Amazon S3 bucket so that it is accessible by clients using either the NFS or SMB protocol\. Files written to a file share become objects in Amazon S3, with the path as the key\. There is a one\-to\-one mapping between files and objects, and the gateway asynchronously updates the objects in Amazon S3 as you change the files\. Existing objects in the bucket appear as files in the file system, and the key becomes the path\. Objects are encrypted with with Amazon S3–server\-side encryption keys \(SSE\-S3\)\. All data transfer is done through HTTPS\.
